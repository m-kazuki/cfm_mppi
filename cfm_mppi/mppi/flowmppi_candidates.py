...
class FlowMPPI(nn.Module):
    def __init__(
        ...
        # velocity = goal / total_time
        # self.prev_optimal_action_seq = velocity.unsqueeze(0).repeat(horizon, 1)
    ...
    def forward(self, state, controls_sin, horizon, goal, obstacle_state, rad, d=0.1, k_p=2.0) -> torch.Tensor:
        ...
        controls_uni = torch.zeros_like(controls_sin).to(self._device)
        ...
        for t in range(horizon):
            ...
            controls_uni[:, t, :] = control_dyn
            state_seq_batch[:, t + 1, :] = self._dynamics(
                state_seq_batch[:, t, :], control_dyn
            )

        initial_prev_action = torch.zeros_like(controls_uni[:, :1, :], device=state_seq_batch.device)
        prev_actions = torch.cat([initial_prev_action, controls_uni[:, :-1, :]], dim=1)
        ...
        if self.prev_optimal_action_seq is not None:
            costs += 0.1 * torch.sum(
                (self.prev_optimal_action_seq.unsqueeze(0)[:,1:,:] - controls_uni) ** 2, dim=(1, 2)
            )
        ...
        mean_action_seq = controls_uni[topk_indices]  # [n_elite, horizon, dim_control]
        ...
        # rollout samples in parallel
        state_seq_batch2 = torch.zeros(n_batch, horizon + 1, self._dim_state).to(state.device)
        state_seq_batch2[:, 0, :] = state.repeat(n_batch, 1)

        for t in range(horizon):
            state_seq_batch2[:, t + 1, :] = self._dynamics(
                state_seq_batch2[:, t, :], perturbed_action_seqs[:, t, :]
            )
        ...
        terminal_costs = self._terminal_cost(state_seq_batch2[:, -1, :2], goal)
        ...
        state_seq_batch3 = torch.zeros(n_elite, horizon + 1, self._dim_state).to(state.device)
        state_seq_batch3[:, 0, :] = state.repeat(n_elite, 1)

        for t in range(horizon):
            state_seq_batch3[:, t + 1, :] = self._dynamics(
                state_seq_batch3[:, t, :], optimal_action_seq[:, t, :]
            )
        initial_prev_action = torch.zeros_like(optimal_action_seq[:, :1, :], device=state_seq_batch3.device)
        prev_actions = torch.cat([initial_prev_action, optimal_action_seq[:, :-1, :]], dim=1)
        times = torch.arange(horizon, device=state_seq_batch3.device)
        vectorized_cost = torch.vmap(
            self.cost_func_for_mode,
            in_dims=(1, 1, 1, 1, 0, None, None),
            out_dims=1
        ...
        terminal_costs = self._terminal_cost(state_seq_batch3[:, -1, :2], goal)
        costs = (
            torch.sum(stage_costs, dim=1)
            + terminal_costs
        )
        ...
        return optimal_action_seq, mean_action_seq_sin, state_seq_batch.transpose(1,2), state_seq_batch[topk_indices].transpose(1,2), state_seq_batch2.transpose(1,2), state_seq_batch3.transpose(1,2)[idx]
