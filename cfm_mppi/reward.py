...
def single_cbf_reward_fn_pairwise(ego_controls, obst_states, obst_controls, r, unicycle=False, init_state=None, a_cbf=1.0, k=5,):
...
def single_collision_reward_fn(ego_controls, obst_states, r):
    ego_states = torch.cumsum(ego_controls*0.1, dim=1)

    diff = ego_states.unsqueeze(0) - obst_states
    distances = torch.norm(diff, dim=1) # (batch_size, num_obstacles, T)
    collision_cost = torch.relu(r-distances)
    return -collision_cost.sum()

def single_goal_reward_fn(ego_controls, goal, unicycle=False, init_state=None):
    ego_states = torch.cumsum(ego_controls*0.1, dim=1)
    goal_dist = torch.norm(ego_states[:2,-1] - goal)
    return -goal_dist.sum()
